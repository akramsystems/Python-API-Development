...
def test_vote_twice_on_post_success(authorized_client, test_posts, test_vote):
...
def test_vote_non_existent(authorized_client, test_posts):
    res = authorized_client.post("/vote/", json={"post_id": 69, "dir": 1})

    assert res.status_code == 404


def test_delete_vote_non_existent(authorized_client, test_posts):
    res = authorized_client.post("/vote/", json={"post_id": 69, "dir": 0})
